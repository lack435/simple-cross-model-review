...
| `changes_requested` | `open_findings`, `verdict_contradiction`, `evidence_incomplete` | Act on `findings` (for `evidence_incomplete`, the reviewer was not shown the whole current change), re-review the same session. |
| `escalate` | `reviewer_blocked` | Stop; the reviewer's own judgement needs a person. Re-reviewing will keep producing this. |
