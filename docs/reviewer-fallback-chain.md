...
pub struct Config {
    ...
    //     codex_fast_mode, allowed_tools, tools, review_preamble, consult_preamble,
    //     isolate_reviewer, metrics, diff, vcs, ...).
}
```

**What stays global, and why.** Only the reviewer's *identity* is per-entry. The
behaviour flags — `--sandbox` and `--codex-fast-mode` (Codex-only), `--tools`/`--allow-tools`
(Claude-only), `--allow-reviewer-config`,
`--review-preamble-file`/`--consult-preamble-file` — stay process-global. This is correct rather
